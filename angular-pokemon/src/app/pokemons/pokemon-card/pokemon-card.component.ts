...
@Component({
  ...
  template: `
    <div class="card">
      <div class="image">
        <img [src]="pokemon.frontShiny" width="80" height="80" alt="pokemon image" />
      </div>
      <div class="details">
        <label for="id">
        ...
        <label for="name">
          <span>Name: </span>
          <a [routerLink]="['pokemon', pokemon.id]" [state]="{ pokemon }"
            ><span id="name" name="name">{{ pokemon.name }}</span></a
          >
        </label>
        <label for="weight">
